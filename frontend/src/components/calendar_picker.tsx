            ...
            <div className="bg-gray-50 rounded-lg p-4">
                ...
                <div className="grid grid-cols-7 gap-2 mb-4">
                    ...
                    {Array.from({ length: DatesLibrary.getDaysInMonth(calendarDate) }).map((_, i) => {
                        ...
                        const hasIndicator = showIndicator ? showIndicator(date) : (getWorkoutsForDate ? getWorkoutsForDate(date).length > 0 : false);

                        return (
                            <button
                                key={day}
                                onClick={() => handleSelectDay(day)}
                                className={`aspect-square rounded-lg font-medium text-sm flex items-center justify-center transition-colors ${selected
