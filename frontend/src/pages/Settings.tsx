...
const Settings: FC = () => {
  ...
  const { data: settings, isLoading: loadingSettings, error, isSuccess } = useQuery<Settings, Error>({
    queryKey: ['settings'],
    queryFn: async () => {
  ...
  const { mutate: updateBodyweightMutation, isPending: updateBodyweightIsPending } = useMutation({
    mutationFn: async (newBodyweight: number) => {
      const res = await apiClient.post("/settings/bodyweight", { bodyweight: newBodyweight });
      return res.data;
    },
    onSuccess: (data) => {
      settings && queryClient.setQueryData<Settings>(['settings'], {
        ...settings,
